...
func (billDao *BillDao) CreateBill(m *models.Bill) (*models.Bill, error) {
    insertQuery := "INSERT INTO bills(BillAdd, Rate) values(?, ?)"
    res, err := billDao.sqlClient.DB.Exec(insertQuery, m.BillAdd, m.Rate)
    if err != nil {
...
func (billDao *BillDao) UpdateBill(id int64, m *models.Bill) (*models.Bill, error)  {
    ...
    updateQuery := "UPDATE bills SET BillAdd = ?, Rate = ? WHERE Id = ?"
    res, err := billDao.sqlClient.DB.Exec(updateQuery, m.BillAdd, m.Rate, id)
    if err != nil {
        return nil, err
    }
...
func (billDao *BillDao) ListBills() ([]*models.Bill, error) {
    ...
    for rows.Next() {
        ...
        if err = rows.Scan(&m.Id, &m.BillAdd, &m.Rate); err != nil {
            return nil, err
        }
...
func (billDao *BillDao) GetBill(id int64) (*models.Bill, error) {
    ...
    if err := row.Scan(&m.Id, &m.BillAdd, &m.Rate); err != nil {
