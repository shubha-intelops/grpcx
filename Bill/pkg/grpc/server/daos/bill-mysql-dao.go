...
func (billDao *BillDao) CreateBill(m *models.Bill) (*models.Bill, error) {
	insertQuery := "INSERT INTO bills(Amount, Name) values(?, ?)"
	res, err := billDao.sqlClient.DB.Exec(insertQuery, m.Amount, m.Name)
	if err != nil {
...
func (billDao *BillDao) UpdateBill(id int64, m *models.Bill) (*models.Bill, error) {
	...
	updateQuery := "UPDATE bills SET Amount = ?, Name = ? WHERE Id = ?"
	res, err := billDao.sqlClient.DB.Exec(updateQuery, m.Amount, m.Name, id)
	if err != nil {
		return nil, err
	}
...
func (billDao *BillDao) ListBills() ([]*models.Bill, error) {
	...
	for rows.Next() {
		m := models.Bill{}
		if err = rows.Scan(&m.Id, &m.Amount, &m.Name); err != nil {
			return nil, err
		}
...
func (billDao *BillDao) GetBill(id int64) (*models.Bill, error) {
	...
	m := models.Bill{}
	if err := row.Scan(&m.Id, &m.Amount, &m.Name); err != nil {
