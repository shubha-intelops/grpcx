...
func (billDao *BillDao) CreateBill(m *models.Bill) (*models.Bill, error) {
	insertQuery := "INSERT INTO bills(Name, Ammount) values(?, ?)"
	res, err := billDao.sqlClient.DB.Exec(insertQuery, m.Name, m.Ammount)
	if err != nil {
...
func (billDao *BillDao) UpdateBill(id int64, m *models.Bill) (*models.Bill, error) {
	...
	updateQuery := "UPDATE bills SET Name = ?, Ammount = ? WHERE Id = ?"
	res, err := billDao.sqlClient.DB.Exec(updateQuery, m.Name, m.Ammount, id)
	if err != nil {
		return nil, err
	}
...
func (billDao *BillDao) ListBills() ([]*models.Bill, error) {
	...
	for rows.Next() {
		m := models.Bill{}
		if err = rows.Scan(&m.Id, &m.Name, &m.Ammount); err != nil {
			return nil, err
		}
...
func (billDao *BillDao) GetBill(id int64) (*models.Bill, error) {
	...
	m := models.Bill{}
	if err := row.Scan(&m.Id, &m.Name, &m.Ammount); err != nil {
